/**
 * /stats/decks — full ranked list of decks for the active scope.
 *
 * Reached from the "ALL" link in the Stats dashboard's Top Decks section.
 * Honours the `group_id` query param so the dashboard's pod scope is
 * preserved on navigation, and the in-screen scope picker lets the user
 * switch pods without going back.
 */
...
export default function StatsAllDecksScreen() {
  ...
  // Mirror the dashboard's "1 pod auto-select" behaviour the first time
  // the user lands here with no explicit scope.
  useEffect(() => {
  ...
  return (
    <SafeAreaView style={styles.screen} edges={['left', 'right', 'bottom']}>
      <StatsAllScreenHeader
        ...
        subtitle={t('stats.subtitleAllDecks')}
      />
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          <Text style={styles.empty}>{t('stats.noRankedDecks')}</Text>
        </View>
      ) : (
        <FlatList
          data={decks}
          keyExtractor={(entry) => entry.deck.id}
          contentContainerStyle={styles.list}
          renderItem={({ item, index }) => (
            <DeckRankRow
              rank={index + 1}
              deck={item.deck}
              commanders={item.commanders}
              matches={item.total_matches}
              win_rate_pct={item.win_rate_pct}
              ownerName={item.commanders[0]?.name ?? null}
              onPress={() => router.push(`/decks/${item.deck.id}`)}
            />
          )}
